...
1. **Sharp Empirical Separatrix:** Non-linear change-point fitting reveals an exceptionally sharp empirical separatrix ($k \in [58.2, 72.4] \gg 15.0, R^2 > 0.91$), decisively falsifying smooth dose-response regularizers ($k < 5.0$).
...
6. **Cross-Benchmark & Architecture Universality:** The critical boundary $\hat{\lambda}_{\text{crit}} \in [0.0165, 0.0245]$ is invariant across synthetic algebraic grammars, natural language semantic parsing, capacity scaling, and recurrent inductive biases.
7. **Hessian Spectral Bounds:** Matrix-free Lanczos iterations show top Hessian eigenvalues strictly bounded below the Edge of Stability ceiling ($\lambda_{\text{max}} \le 1680.4 \ll 2/\eta = 2000.0$), confirming stable continuous manifold reorganization.
---
...
4. [x] LLM writing assistance disclosed in paper footnote (`\thanks`).
5. [x] Open-source repository: \url{https://github.com/basyirinbasri/sigma-model} (Release \texttt{v2.0-paper02}, Commit \texttt{69e1f57b}) and reproducibility bundle (\texttt{supplementary\_materials.zip}). Permanent Zenodo community archive will be formally minted upon publication.
