...
1. **Model & Benchmark Specifications:**
   - Architecture: 2-layer standard seq2seq Transformer ($d_{\text{model}} = 128, n_{\text{heads}} = 4, d_{\text{ff}} = 512, n_{\text{layers}} = 2$). Implemented in `paper02/src/models/seq2seq_transformer.py`.
   - Grammar: H-Bar zero-leakage compositional dataset ($N_{\text{train}} = 10,000, N_{\text{OOD}} = 2,000$). Implemented in `paper02/src/data/hbar_dataset.py`.
...
2. **Experimental Grid Parameters ($n = 30$ seeds per cell):**
   ...
   - *Total Gate Runs:* 450 runs (configured in `paper02/experiments/configs/gate_protocol.yaml`).

### Task 3.2: Self-Contained Gate Runner Implementation [COMPLETE]
1. [x] Write self-contained execution harness: `paper02/src/experiments/run_gate.py`.
...
4. [x] Implement automatic output serialization to `paper02/experiments/YYYY-MM-DD_gate/all_results.pkl`.
...
1. Execute analysis script: `paper02/src/analysis/analyze_gate.py`.
2. Compile and emit `paper02/planning/gate-result.md`:
...
- Decision record (Design): `paper02/decisions/ADR-005_gate_experimental_design.md`
- Pre-registration: `paper02/planning/preregistration.md`
- Protocol configuration: `paper02/experiments/configs/gate_protocol.yaml`
- Model & data modules: `paper02/src/models/seq2seq_transformer.py`, `paper02/src/data/hbar_dataset.py`
- Runner script: `paper02/src/experiments/run_gate.py`
- Raw data bundle: `paper02/experiments/YYYY-MM-DD_gate/all_results.pkl` (+ manifests)
- Gate document: `paper02/planning/gate-result.md`
- Directive record: `paper02/decisions/human-gates/P03_gate_directive.md`
