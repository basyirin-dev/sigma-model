...
- **Prerequisites:** 
  - `paper02/decisions/ADR-006_gate_verdict_and_threshold_reconciliation.md` (Gate passed, $\hat{\lambda}_{\text{crit}} = 0.025 \pm 0.005$)
  - `paper02/planning/preregistration.md`
...
| **Arch C** | Gated Recurrent Seq2Seq (GRU) | 128 | 2 | N/A | ~0.6M | Tests cross-architecture universality beyond attention mechanisms (ADR-012). |
...
1. **Implementation:** Codify declarative matrix configuration in `paper02/experiments/configs/matrix_p04.yaml` and manifest generator in `paper02/src/config/matrix.py`.
2. **Verification:** Add unit tests validating parameter coverage and seed uniqueness in `paper02/tests/test_matrix.py`.
