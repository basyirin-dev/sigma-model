...
The original manuscript claims all component functions are
$C^\infty$ on $\text{int}(\mathcal{X})$. This is false for two terms:
\begin{enumerate}[nosep]
\item The Stage~1 fusion equation ($\tilde{\sigma}_A = w_g \max(0, g_A) + w_r \max(0, r_A) + w_c c_A$) contains $\max(0, g_A)$,
  which is not differentiable at $g_A = 0$.
\item The RGA signal (defined in the manuscript) uses absolute cosine similarity
