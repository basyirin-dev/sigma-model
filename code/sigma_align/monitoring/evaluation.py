...
def score_axe1() -> AxeResult:
    ...
    if ode_sol.exists():
        ...
        subchecks["solver has sigma clamping"] = (
            "sigma" in sol_src.lower() and "clip" in sol_src.lower()
        )
...
def score_axe2() -> AxeResult:
    ...
    if nb_path.exists():
        ...
        subchecks["notebook has statistical reporting"] = (
            "p-" in nb_text.lower() or "cohen" in nb_text.lower()
            or "effect size" in nb_text.lower()
        )
        subchecks["notebook has ID/OOD comparison"] = (
            "ood" in nb_text.lower() and "id" in nb_text.lower()
        )
    ...
    if metrics_file.exists():
        ...
        subchecks["metrics module has effect-size helpers"] = (
            "compute_reliability" in mtext or "ood_ratio" in mtext
        )
...
def score_axe3() -> AxeResult:
    ...
    if proof_dir.exists():
        ...
        for tf in tex_files:
            if "circularity" in tf.read_text().lower():
                circular_aware += 1
        subchecks[
            f"circularity-check sections ({circular_aware}/{len(tex_files)})"
        ] = circular_aware >= 3
        ...
        subchecks[
            f"boundary-case sections ({boundary_aware}/{len(tex_files)})"
        ] = boundary_aware >= 3
    ...
    if CLAIMS_FILE.exists():
        ...
        subchecks["claims registry has entries"] = n_claims >= 10
...
def score_axe5() -> AxeResult:
    ...
    if training_py.exists():
        ...
        subchecks["deterministic seed set in training"] = (
            "manual_seed" in tr_text or "torch.manual_seed" in tr_text
        )
        subchecks["cudnn deterministic configured"] = (
            "cudnn.deterministic" in tr_text or "cudnn.benchmark" in tr_text
        )
...
def score_axe6() -> AxeResult:
    ...
    if CLAIMS_FILE.exists():
        ...
        subchecks["has verification status"] = (
            "verified" in claims_text.lower() or "PROVEN" in claims_text
        )
    ...
    if ms.exists():
        ...
        details_parts.append(
            f"claim anchors in manuscript: {n_anchors} "
            f"({len(latex_anchors)} LaTeX, {len(html_anchors)} HTML)"
        )
...
def score_axe7() -> AxeResult:
    ...
    for name, cfg_key in benchmarks.items():
        ...
        if cfg_path.exists():
            ...
            subchecks[f"{name} has benchmark field"] = (
                name in text or cfg_key.replace("-", "_") in text
            )
    ...
    if ms.exists():
        ...
        for name in benchmarks:
            subchecks[f"{name} mentioned in paper"] = (
                name in ms_text or name.replace("-", "") in ms_text
            )
