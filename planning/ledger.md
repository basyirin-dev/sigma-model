...
**Status:** Active — P00–P05 closed (`p05-implemented`); **P06 BLOCKED_ON_REMEDIATION (production not authorized)**
...
| **CLM-003** | Empirical | **Empirical Separatrix:** In deep Transformers, the empirical escape probability $P(\text{escape} \mid \lambda, t)$ displays a sharp phase boundary at $\hat{\lambda}_{\text{crit}}$ rather than a smooth dose-response curve. | Phase 03 Gate Target ($k=200.00 \ge 15.0$, $\hat{\lambda}_{\text{crit}}=0.025$, ADR-006) | `keep` | {experiment, 0.99, Gate 2026-08-24} | 5 | 5 | CLM-001 | P03 (Gate Passed) |
| **CLM-004** | Empirical | **Late-Onset Recovery:** Activating supercritical pressure $\lambda > \lambda_{\text{crit}}$ after the model is trapped in $E_S$ destabilizes the shortcut state and restores OOD generalization. | Phase 03 Gate Target ($100\%$ recovery at $t_{\text{int}}=1000$, ADR-006) | `keep` | {experiment, 0.99, Gate 2026-08-24} | 5 | 5 | CLM-001 | P03 (Gate Passed) |
| **CLM-005** | Diagnostic | **Geometric Lead-Lag:** Centered Kernel Alignment (CKA / RGA) Granger-causes subsequent OOD accuracy improvements ($\Delta \text{RGA}_t \to \Delta \text{OOD}_{t+1}$). | Inherited from Paper 01 finding & manifold geometry (LIT-05); preregistration exploratory | `keep` | {experiment, 0.85, P03/P07} | 3 | 4 | CLM-003 | P07 |
| **CLM-006** | Diagnostic | **Whitened GCA Correction:** Projecting out the shared input embedding subspace eliminates the step-0 $0.95$ GCA artifact. | Mathematical fix for GCA initialization artifact (LIT-04) | `keep` | {analytical, 0.95, ADR-010} | 3 | 4 | — | P05 (ADR-010) |
