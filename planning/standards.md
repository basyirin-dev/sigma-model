# Cross-Cutting Standards & Scientific Governance (RPF v2.0)

This document establishes the mandatory cross-cutting rules (**CC.N.M**) that govern all phases of research in this repository. Compliance is verified automatically via `scripts/compliance_linter.py` and `scripts/generate_compliance_matrix.py`.

---

## CC.1: Reproducibility & Environment Control
...
## CC.2: Numerical Integrity & Solver Tolerances

### Numerical Solver Tolerance Table
| Regime / Equation Type | Recommended Solver | Absolute Tolerance (`atol`) | Relative Tolerance (`rtol`) | Max Step Size (`max_dt`) |
|---|---|---|---|---|
| Non-stiff ODE Baseline | Tsit5 / RK45 | `1e-10` | `1e-10` | `1e-2` |
| Stiff σ-Trap Bifurcations | RadauIIA5 / KenCarp4 | `1e-10` | `1e-10` | `1e-3` |
| SDE Continuous Models | SRIW1 / Euler-Maruyama | `1e-6` | `1e-6` | `1e-4` |
| Lanczos Hessian Spectrum | PyHessian Lanczos | `1e-6` | `1e-6` | N/A (Iter: 100) |
| CKA / Representation Probing | Whitened GCA / Linear CKA | `1e-12` (Eps) | `1e-12` | N/A |
...
## CC.3: Writing Quality, Precision & Claim Discipline
...
- **CC.3.4:** All empirical claims must report sample sizes ($N \ge 5$ random seeds), central tendency, confidence intervals (95% CI or bootstrap), and exact p-values.
...
## CC.4: Data Hygiene, Immutability & Provenance
...
- **CC.4.3:** Data leakage audits must be executed to ensure zero overlap between train, validation, and out-of-distribution (OOD) test splits.
...
- **CC.4.7:** Checksum integrity (SHA-256) of all input datasets must be verified before experiment execution.
- **CC.4.8:** Data ablation splits must be pre-stratified and indexed deterministically.

---

## CC.5: Release-Readiness & Packaging
...
## CC.6: Agentic Protocol, Handoffs & State Serialization
...
## CC.7: Ethics, Dual-Use & Safety Compliance
