...
> **Governance:** Zero writing without committed evidence. Strict compliance with `paper/planning/standards.md`.
...
2. **Evidence Gates Ambition (RPF Principle 1):**
   Under no circumstances should the agent draft manuscript sections (P08) or run multi-benchmark sweeps (P06) before `paper/planning/gate-result.md` has been committed with an approved **PASS** verdict.
3. **Artifact-Gated Commits:**
   Every technical decision must produce an ADR in `paper/decisions/`.
   Every claim must trace to a row in `paper/planning/ledger.md`.
...
| Path | Purpose & Rules |
|---|---|
| `paper/planning/` | Phase briefs, temporal roadmap, cross-cutting standards, claim ledger. Immutable structure. |
| `paper/decisions/` | Numbered Architecture Decision Records (`ADR-00X_...`). |
| `paper/src/` | Pure Python/JAX/PyTorch implementation modules. All custom numerical kernels must have unit tests. |
| `paper/data/raw/` | Raw tensor outputs from experimental runs. **Immutable after P06 data generation.** |
| `paper/data/processed/` | Derived tidy parquet/csv files generated exclusively by idempotent scripts. |
| `paper/writing/` | LaTeX source files for the manuscript, TikZ vector figures, and supplementary materials. |
| `paper/meta/` | Operational guidelines, environment specifications, and project README. |
...
- **Linting:** `ruff check paper/src/`
- **Unit Tests:** `pytest paper/tests/`
- **Manuscript Build:** `make -C paper pdf` (Must yield 0 errors, 0 undefined citations).
