...
1. **Deterministic Seeding Protocol:**
   - Seed Derivation Formula: `seed = cell_seed_idx * 42 + 7` where `cell_seed_idx` is the within-cell replicate index ($0 \dots 29$ for Tier 1 with $n=30$; $0 \dots 9$ for Tier 2 with $n=10$), strictly disambiguated from global 1-indexed `run_id` ($1 \dots 960$).
   - PyTorch: `torch.manual_seed(seed)`
   - JAX: `jax.random.PRNGKey(seed)`
   - NumPy / Python: `np.random.seed(seed)`, `random.seed(seed)`
...
3. **Artifact Immutability:**
   - Raw simulation tensors generated during Phase 06 are saved as immutable Parquet/NPZ files in `paper/data/raw/` with SHA-256 integrity logs.
