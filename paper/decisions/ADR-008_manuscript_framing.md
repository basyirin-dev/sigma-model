# ADR-008: Scientific Manuscript Framing, Theoretical Positioning & Evidence Synthesis
...
- **Driver 3 (Bidirectional Ledger Synchronization):** Ensure all in-scope claims (CLM-001 through CLM-007) in `paper/planning/ledger.md` are completely grounded in the text, tables, and figures, while deferred claims (CLM-008 through CLM-012) are formally scoped to future research.
...
- **Compilation Status:** `make -C paper pdf` exits with code 0; generates 18-page publication PDF (`paper/writing/manuscript.pdf`).
...
- **Code & Figure Provenance:** All 5 publication figures in `paper/writing/figures/` are generated from verified data tables with complete JSON metadata sidecars.
