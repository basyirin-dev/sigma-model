...
**Deliverables:** `paper/meta/*`, `paper/planning/roadmap.md`, `paper/decisions/ADR-template.md`, `paper/Makefile`, `paper/src/__init__.py`
...
3. **Generate Environment Spec:**
   - Emit `paper/meta/ENVIRONMENT.md` recording exact package hashes, CUDA drivers, and CPU/GPU specifications.
...
1. **Scaffold Core Meta Documents:**
   - Write `paper/meta/README.md` (project synopsis and quickstart commands).
   - Write `paper/meta/AGENT_INSTRUCTIONS.md` (detailed operating manual, guardrails, and the 14 Human-Gate rules; $\ge 50$ lines).
2. **Scaffold Architecture Decision Records (ADRs):**
   - Create `paper/decisions/ADR-template.md`.
   - Initialize `paper/decisions/ADR-001_two_subspace_formulation.md`.

### Task 0.5: Build System & Smoke Testing
1. **Makefile Setup:**
   - Create `paper/Makefile` supporting targets `pdf`, `clean`, `test`, `figures`, and `arxiv`.
   - Update root `Makefile` to include `paper` target forwarding to `paper/Makefile`.
2. **Automated Smoke Test:**
   - Create `paper/tests/test_env_smoke.py` verifying that PyTorch, JAX, Diffrax, SciPy, and CKA routines import and execute a dummy tensor operation with exit code 0.
...
- [ ] `paper/planning/roadmap.md` exists with populated header, deliverable matrix, and non-negotiables.
- [ ] `paper/planning/standards.md` exists with frozen CC.N.M rules.
- [ ] `paper/planning/ledger.md` exists with initial claims ledger.
- [ ] `paper/meta/AGENT_INSTRUCTIONS.md` exists and is $\ge 50$ lines.
- [ ] `paper/meta/ENVIRONMENT.md` documents all pinned dependencies.
- [ ] `python -c "import torch, jax, diffrax, scipy, geomstats; print('Imports PASS')"` exits with code 0.
- [ ] `pytest paper/tests/test_env_smoke.py` exits with code 0.
- [ ] `[HUMAN-GATE]` User confirms environment readiness.

---

## 5. Deliverables & Artifacts
- Scaffolding: `paper/planning/`, `paper/decisions/`, `paper/src/`, `paper/meta/`, `paper/writing/`
- Documentation: `paper/meta/README.md`, `paper/meta/AGENT_INSTRUCTIONS.md`, `paper/meta/ENVIRONMENT.md`, `paper/decisions/ADR-template.md`
- Build files: `paper/Makefile`, `paper/tests/test_env_smoke.py`
