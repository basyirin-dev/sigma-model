...
\author{Anonymous (narrow paper: ``The $\sigma$-Trap: A Dynamical Model of
Schema-Coherence Suppression in Compositional Generalisation'')}
\date{2026-08-16}
...
\begin{abstract}
This technical report is the companion to the narrow paper ``The
$\sigma$-Trap: A Dynamical Model of Schema-Coherence Suppression in
Compositional Generalisation'' (Anonymous, 2026). It preserves the extended
$\Sigma$-Model framework that the narrow paper deliberately leaves out:
...
The narrow paper (Anonymous, 2026) develops a two-variable dynamical model
...
Figure~the estimation figure of the narrow paper illustrates the two-stage estimation architecture.
...
\State \textbf{Integrate ODE:} Update $(\delta_A, \sigma_A, \alpha_A, \hat{M}_A, \Xi_A)$ via Algorithm~the integration algorithm
...
The full training pipeline is shown in Figure~the pipeline figure of the narrow paper.
...
where the two variables dissociate, as illustrated in Figure~the diagnostic figure of the narrow paper. Standard $2\times 2$ factorial for
Learning track:
...
Figure~Figure~4 of the narrow paper visualises the bifurcation structure and phase portrait.
...
Each prediction below is derived from the ODE system and is distinguished from depth-only ($\delta_A$-only) accounts. Predictions~1--8 constitute the eight core pre-registered comparisons; Prediction~9 provides a computational ODE validation, and Meta-Prediction~M1 synthesises the core predictions into an overarching claim about intervention mechanisms. Section~the narrow paper presents pilot data for Predictions~1,~6, and~9.
...
(see \S~the conclusion of the narrow paper, item~2).
