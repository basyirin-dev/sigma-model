"""Turnkey Partitioned Kaggle Notebook Generator for Paper 02 (Phase 06).

Generates 3 partitioned, standalone Kaggle notebooks adhering to hard Kaggle constraints:
- Max session duration < 12 hours (each notebook runs in ~1.5 - 2.5 hours).
- Host RAM < 13 GB via aggressive gc.collect() and cuda.empty_cache() after every run.
- Scratch disk < 5 GB by serializing only compact trajectories and spectral metrics (< 30 MB).
- 100% self-contained standard PyTorch/NumPy/SciPy with zero external pip dependencies.
- Zero internet requirement: all benchmark datasets embedded and generated in-memory.
- Preflight Self-Audit Cell: verifies zero-leakage, stream distinction, and micro-bifurcation before running main sweeps!

Partitions:
1. Part 1 (Tier 1A): H-Bar & SCAN (360 runs, n=30, ~1.3h GPU)
2. Part 2 (Tier 1B): COGS & PCFG-SET (360 runs, n=30, ~1.3h GPU)
3. Part 3 (Tier 2): Exploratory Architecture Scaling (4L Transformer & GRU, 240 runs, n=10, ~0.9h GPU)
"""
...
def build_common_preamble_cells(part_title: str, part_desc: str) -> list[dict[str, Any]]:
    """Build markdown and setup cells common to all partitioned notebooks."""
    md_cell = {
        "cell_type": "markdown",
        "metadata": {},
        "source": [
            f"# Σ-Model Paper 02 — Phase 06: {part_title}\n",
            "\n",
            f"{part_desc}\n",
            "\n",
    ...
    return [md_cell, setup_cell]


def build_data_and_arch_cells() -> list[dict[str, Any]]:
    """Build dataset tokenizers, dataset loaders, and model definitions."""
    benchmark_cell = {
        "cell_type": "code",
        "execution_count": None,
        "metadata": {},
        ...
        "source": [
            ...
            "        # Output primitives\n",
            "        'I_WALK', 'I_LOOK', 'I_RUN', 'I_JUMP', 'I_TURN', 'I_TURN_LEFT', 'I_TURN_RIGHT',\n",
            "        'I_PUSH', 'I_PULL', 'I_LIFT', 'I_AND', 'I_AFTER',\n",
            "        'I_LEFT', 'I_RIGHT', 'I_TWICE', 'I_THRICE', 'I_OPPOSITE', 'I_AROUND',\n",
            "    ]\n",
            ...
            "        # Homomorphic Command Algebra (Tree depth d <= 3 train, d in {4,5} test)\n",
            "        for a in actions:\n",
            "            for d in ['left', 'right', 'around']:\n",
            "                train_pairs.append(([a, d], [f'I_{a.upper()}', f'I_{d.upper()}']))\n",
            "                for m in ['twice', 'thrice', 'opposite']:\n",
            "                    train_pairs.append(([m, a, d], [f'I_{m.upper()}', f'I_{a.upper()}', f'I_{d.upper()}']))\n",
            "                    train_pairs.append(([a, d, m], [f'I_{a.upper()}', f'I_{d.upper()}', f'I_{m.upper()}']))\n",
            "        for a1 in actions:\n",
            "            for d1 in ['left', 'right']:\n",
            "                for c in combs:\n",
            "                    for a2 in actions:\n",
            "                        for d2 in ['left', 'right']:\n",
            "                            train_pairs.append(([a1, d1, c, a2, d2], [f'I_{a1.upper()}', f'I_{d1.upper()}', f'I_{c.upper()}', f'I_{a2.upper()}', f'I_{d2.upper()}']))\n",
            "        for a in actions:\n",
            "            for d in ['left', 'right']:\n",
            "                comp_pairs.append(([a, d], [f'I_{a.upper()}', f'I_{d.upper()}']))\n",
            "                comp_pairs.append(([a, d, 'twice'], [f'I_{a.upper()}', f'I_{d.upper()}', 'I_TWICE']))\n",
            "        for a1 in actions:\n",
            "            for d1 in ['left', 'right']:\n",
            "                for m1 in ['twice', 'thrice']:\n",
            "                    for c in combs:\n",
            "                        for a2 in actions:\n",
            "                            for d2 in ['left', 'right']:\n",
            "                                for m2 in ['opposite']:\n",
            "                                    ood_pairs.append(([m2, a1, d1, m1, c, a2, d2], [f'I_{m2.upper()}', f'I_{a1.upper()}', f'I_{d1.upper()}', f'I_{m1.upper()}', f'I_{c.upper()}', f'I_{a2.upper()}', f'I_{d2.upper()}']))\n",
            "    \n",
            "    elif benchmark_name == 'scan_jump':\n",
            "        # Lake & Baroni SCAN grammar: jump appears strictly in atomic context in train\n",
            "        for a1 in actions:\n",
            "            for m1 in mods:\n",
            "                for c in combs:\n",
            "                    for a2 in actions:\n",
            ...
            "        # Kim & Linzen / ReCOGS semantic parsing\n",
            ...
            "        # Hupkes et al. context-free production expansions (systematicity Cartesian substitution)\n",
            "        nouns = ['box', 'table', 'ball', 'cat', 'dog']\n",
            "        verbs = ['push', 'pull', 'lift', 'look']\n",
            "        held_out_pairs = {('push', 'box'), ('pull', 'table'), ('lift', 'ball')}\n",
            "        for v in verbs:\n",
            "            for n in nouns:\n",
            "                if (v, n) not in held_out_pairs:\n",
            "                    train_pairs.append(([v, n], [f'I_{v.upper()}', 'theme', n]))\n",
            "                    for c in combs:\n",
            "                        for v2 in verbs:\n",
            "                            for n2 in nouns:\n",
            "                                if (v2, n2) not in held_out_pairs:\n",
            "                                    train_pairs.append(([v, n, c, v2, n2], [f'I_{v.upper()}', 'theme', n, f'I_{c.upper()}', f'I_{v2.upper()}', 'theme', n2]))\n",
            "        for (v, n) in held_out_pairs:\n",
            "            comp_pairs.append(([v, n], [f'I_{v.upper()}', 'theme', n]))\n",
            "            for v2 in verbs:\n",
            "                for n2 in nouns:\n",
            "                    if (v2, n2) not in held_out_pairs:\n",
            "                        ood_pairs.append(([v, n, 'and', v2, n2], [f'I_{v.upper()}', 'theme', n, 'I_AND', f'I_{v2.upper()}', 'theme', n2]))\n",
            "                        ood_pairs.append(([v2, n2, 'after', v, n], [f'I_{v2.upper()}', 'theme', n2, 'I_AFTER', f'I_{v.upper()}', 'theme', n]))\n",
            "    \n",
    ...
    return [benchmark_cell, arch_cell]


def build_telemetry_and_harness_cells() -> list[dict[str, Any]]:
    """Build telemetry, Whitened GCA, Lanczos Hessian, and training harness."""
    probe_cell = {
        "cell_type": "code",
        "execution_count": None,
        "metadata": {},
    ...
    return [probe_cell, train_cell, preflight_cell]


def build_execution_cell(
    part_name: str,
    benchmarks: list[str],
    ...
    """Build the parameter loop and execution harness cell for a specific partition."""
    code_str = f'''# ===========================================================================
# 5. Partitioned Execution Master Loop ({part_name})
# ===========================================================================

def execute_partition_matrix():
    benchmarks = {benchmarks!r}
    architectures = {architectures!r}
    lambdas = {lambda_grid!r}
    ...
    print("=" * 75)
    print("🚀 STARTING PARTITION EXECUTION: {part_name}")
    print("=" * 75)
    ...
    for idx, cfg in enumerate(planned_runs, start=1):
        ...
        if idx % 10 == 0 or idx == total_runs:
            ...
            with open(OUT_DIR / "{part_name}_partial.pkl", "wb") as f:
                pickle.dump(all_results, f)

    # Final Serialization
    final_pkl = OUT_DIR / "{part_name}_results.pkl"
    with open(final_pkl, "wb") as f:
        pickle.dump(all_results, f)

    zip_path = Path("/kaggle/working/{part_name}_archive.zip")
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
        for p in OUT_DIR.glob("*.*"):
            zipf.write(p, arcname=p.name)

    print("=" * 75)
    print(f"✅ All {{len(all_results)}} runs complete in {part_name}!")
...
def generate_all_partitioned_notebooks(out_dir: Path | str = "paper/notebooks") -> list[Path]:
    """Generate all 3 partitioned Kaggle production notebooks."""
    ...
    partitions = [
        {
            "filename": "kaggle_p06_part1_tier1_hbar_scan.ipynb",
            "title": "Part 1 (Tier 1A Primary) — H-Bar & SCAN Sweeps",
            "desc": "**Tier 1A Primary Falsification (360 Runs):**\n- Benchmarks: H-Bar, SCAN (`add_primitive_jump`)\n- Architecture: Transformer 2L (`arch_a_transformer_2l`)\n- Grid: 6 λ levels $\\times$ 30 seeds = 360 runs\n- Estimated runtime: ~1.3 hours GPU",
            "part_name": "p06_part1_hbar_scan",
            "benchmarks": ["hbar", "scan_jump"],
            "architectures": ["transformer_2l"],
            "lambda_grid": [0.000, 0.015, 0.020, 0.025, 0.030, 0.500],
            "seeds_per_cell": 30,
            "tier_name": "tier_1_primary_change_point",
            "evidence_class": "primary",
        },
        {
            "filename": "kaggle_p06_part2_tier1_cogs_pcfg.ipynb",
            "title": "Part 2 (Tier 1B Primary) — COGS & PCFG-SET Sweeps",
            "desc": "**Tier 1B Primary Falsification (360 Runs):**\n- Benchmarks: COGS, PCFG-SET\n- Architecture: Transformer 2L (`arch_a_transformer_2l`)\n- Grid: 6 λ levels $\\times$ 30 seeds = 360 runs\n- Estimated runtime: ~1.3 hours GPU",
            "part_name": "p06_part2_cogs_pcfg",
        ...
        {
            "filename": "kaggle_p06_part3_tier2_arch_scaling.ipynb",
            "title": "Part 3 (Tier 2 Exploratory) — Architecture Scaling & Universality",
            "desc": "**Tier 2 Exploratory Universality (240 Runs):**\n- Benchmarks: H-Bar, SCAN, COGS, PCFG-SET (All 4)\n- Architectures: Transformer 4L Scaled (`transformer_4l_scaled`), GRU Baseline (`gru_baseline`)\n- Grid: 3 λ levels ($0.000, 0.025, 0.500$) $\\times$ 10 seeds = 240 runs\n- Estimated runtime: ~0.9 hours GPU",
            "part_name": "p06_part3_arch_scaling",
            "benchmarks": ["hbar", "scan_jump", "cogs", "pcfg_set"],
            "architectures": ["transformer_4l_scaled", "gru_baseline"],
            "lambda_grid": [0.000, 0.025, 0.500],
            "seeds_per_cell": 10,
            "tier_name": "tier_2_exploratory_scaling",
    ...
    for p in partitions:
        cells = []
        cells.extend(build_common_preamble_cells(p["title"], p["desc"]))
        cells.extend(build_data_and_arch_cells())
        cells.extend(build_telemetry_and_harness_cells())
        cells.append(
            build_execution_cell(
                part_name=p["part_name"],
                benchmarks=p["benchmarks"],
                architectures=p["architectures"],
                lambda_grid=p["lambda_grid"],
                seeds_per_cell=p["seeds_per_cell"],
        ...
        fpath = out_path / p["filename"]
        fpath.write_text(json.dumps(nb, indent=2), encoding="utf-8")
        generated_files.append(fpath)
        print(f"✅ Generated Partition Notebook: {fpath}")

    return generated_files


if __name__ == "__main__":
    generate_all_partitioned_notebooks()
